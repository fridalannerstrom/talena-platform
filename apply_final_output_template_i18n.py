#!/usr/bin/env python3
"""
Apply Django i18n tags to the static Final Output interface.
...
Run from the repository root:
    ...
    python apply_final_output_template_i18n.py --check
    python apply_final_output_template_i18n.py --apply
"""
...
def transform(text: str) -> tuple[str, list[str]]:
    updated = add_i18n_preamble(text)
    notes: list[str] = []

    # Longer strings first, so a short phrase cannot alter part of
    # a longer sentence before that sentence is processed.
    ordered = sorted(
        REPLACEMENTS,
    ...
    for replacement in unique:
        updated, count = flexible_replace(
            updated,
            replacement,
        )
